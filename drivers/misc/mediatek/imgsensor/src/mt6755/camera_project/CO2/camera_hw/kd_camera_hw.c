...
PowerUp PowerOnList = {
	{
	 ...
	 {SENSOR_DRVNAME_S5K4H7YXKC_MIPI_RAW,
	  {
	   {SensorMCLK, Vol_High, 1},
	   {DOVDD, Vol_1800, 1},
	   {DVDD, Vol_1200, 2},
...
BOOL hwpoweron(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		...
		mtkcam_gpio_set(pinSetIdx, PDN,1);
#if 0
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		...
		//PK_DBG("hwPowerDown: PDN %d\n", pwInfo.Voltage);
                mtkcam_gpio_set(pinSetIdx, PDN,0);
#if 0
		if (pwInfo.Voltage == Vol_High) {
			if (mtkcam_gpio_set(pinSetIdx, PDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON])) {
				PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
...
#if 0
	if(0 == strcmp(currSensorName,SENSOR_DRVNAME_S5K4H7YX_487C_MIPI_RAW))
		pinSetIdx = 0;
	else if(0 == strcmp(currSensorName,SENSOR_DRVNAME_S5K4H7YX_MIPI_RAW))
		pinSetIdx = 1;
#endif
