...
#define DATA_SIZE		1356
#define START_ADDR		0X0790
BYTE S5K3P9_eeprom_data[DATA_SIZE]= {0};
...
static bool _read_eeprom(kal_uint16 addr, kal_uint32 size ){
	...
	if(read_cmos_sensor_byte(0x0000) == 0x01){
		for(; i<size; i++){
			S5K3P9_eeprom_data[i] = read_cmos_sensor_byte(addr+i);
			datasum += S5K3P9_eeprom_data[i];
			LOG_INF("addr = 0x%x,\tvalue = 0x%x\n",addr+i,S5K3P9_eeprom_data[i]);
		}
		//datasum += read_cmos_sensor_byte(0x0CDC);
		//datasum += read_cmos_sensor_byte(0x0CDD);
		temph = read_cmos_sensor_byte(0x0CDE);
		templ = read_cmos_sensor_byte(0x0CDF);
		LOG_INF("temph = 0x%x,templ = 0x%x\n",temph, templ);
		checksum = ((temph&0xFF) << 8) | (templ&0xFF);

		LOG_INF("datasum = 0x%x,checksum = 0x%x\n",datasum, checksum);
		if(checksum == datasum){
			LOG_INF("[S5K3P9]PDAF checksum ok!\n");
			return true;
		} else {
			LOG_INF("[S5K3P9]PDAF checksum fail!\n");
			return false;
		}
	} else {
		LOG_INF("[S5K3P9] otp is null\n");
		return false;
	}
}
...
bool S5K3P9_read_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size){
	...
	memcpy(data, S5K3P9_eeprom_data, size);
	return true;
}
