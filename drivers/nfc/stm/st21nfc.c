...
static struct pinctrl_state *st_rst_inpd = NULL;
...
static int st_nfc_set_gpio_inpd(void)
{
	pr_info("st_nfc_set_gpio_inpd enter\n");

	if(gpctrl) {
		/*rst_gpio*/
		if(st_rst_inpd) {
			pinctrl_select_state(gpctrl, st_rst_inpd);
		}
...
/* Read data */
#if defined(KERNEL318MTK) && defined(CONFIG_MTK_I2C_EXTENSION)
...
static int st21nfc_platform_probe(struct platform_device *pdev)
{
	...
	/*rst_gpio*/
	st_rst_inpd = pinctrl_lookup_state(gpctrl, "rst_input_pulldown");
	if (IS_ERR(st_rst_inpd)) {
		pr_err("%s : pinctrl err, rst_input_pulldown\n", __func__);
		st_rst_inpd = NULL;
		return 0;
	}
...
static int check_nfc_hw_support(void *x)
{
...
//#endif
	ret = i2c_add_driver(&st21nfc_driver);
	if(0 == ret) {
